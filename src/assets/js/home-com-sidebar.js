...
function insereJogadorBaseCadastro(name, nvl) {
  let novoJogador = {
    nome: name,
    nivel: nvl,
  };
...
function addJogadoresCadastrados(){
  const listaDeJogadores = JSON.parse(localStorage.getItem('db_usuario'))
  let qtd = listaDeJogadores.usuarios.length;
  for(let jogador of listaDeJogadores.usuarios){
    const li = criaLi();
    li.innerText = jogador.nome;
    jogadoresNaTela.appendChild(li); // insere o elemento <li> dentro da <ul class="jogadores">
    insereJogadorBaseCadastro(jogador.nome, jogador.nivel); // chama a função para adicionar o jogador digitado
    criaBotaoApagar(li);
  }
  let inputJogadores = document.getElementById('total-jogadores') 
  inputJogadores.value = qtd;
}

const botaoCadastrados = document.querySelector(".botao-dados");
botaoCadastrados.addEventListener("click", function (event){

  addJogadoresCadastrados()
